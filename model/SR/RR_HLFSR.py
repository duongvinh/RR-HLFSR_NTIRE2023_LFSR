...
class get_model(nn.Module):
	def __init__(self, args):
		...
		# define head module
		modules_head = [nn.Conv2d(1 ,self.channels, kernel_size=3, stride=1,dilation=int(angRes), padding=int(angRes), bias=False)]

		 # define body module
		modules_body = [
			HFEM(angRes, self.n_blocks, self.channels,first=False) \
			for _ in range(self.n_groups)]

		modules_body.append(nn.Conv2d(self.channels, self.channels, kernel_size=3,stride =1,dilation=1,padding=1, bias=False))
